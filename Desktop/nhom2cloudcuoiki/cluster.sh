...
start)  # start [<clusterName>]
  ...
  TARGET="WordCount-result-group2"
  gcloud dataproc jobs submit hadoop --cluster "$CLUSTER" \
    --jar target/wordcount-mapreduce-0-SNAPSHOT-jar-with-dependencies.jar \
    -- wordcount-hbase \
    gs://dataproc-3ccc8cea-d7c3-4554-b1d6-8f02bbac0478-us/word-count-01.txt\
    gs://dataproc-3ccc8cea-d7c3-4554-b1d6-8f02bbac0478-us/word-count-02.txt\

    "${TARGET}"
    echo ""
    echo "Kết quả của table đó là : ${TARGET}"
    echo ""
  ;;
